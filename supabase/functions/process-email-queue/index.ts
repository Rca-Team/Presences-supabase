...
const RESEND_GATEWAY_URL = 'https://api.resend.com'
const MAX_RETRIES = 5
...
async function sendViaResendGateway(
  ...
  if (!response.ok) {
    ...
    const err = new Error(`Resend gateway send failed [${response.status}]: ${errorBody}`)
...
async function sendViaLovable(payload: EmailPayload, apiKey: string): Promise<void> {
  await sendViaResendGateway(payload, apiKey)
}
...
async function sendWithResendFallback(
  ...
  await sendViaResendGateway(payload, resendApiKey)
  return 'resend'
}
